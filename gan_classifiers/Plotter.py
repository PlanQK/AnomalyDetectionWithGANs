...
class Plotter:
    ...
    def loss_plot(self):
        plt.plot(self.train_hist["step_number"],
                 np.asarray(self.train_hist["generator_loss"]) / self.train_hist["generator_loss"][0], label="gen loss")
        plt.plot(self.train_hist["step_number"],
                 np.asarray(self.train_hist["discriminator_loss"]) / self.train_hist["discriminator_loss"][0],
                 label="disc loss")
        plt.plot(self.train_hist["step_number"],
                 np.asarray(self.train_hist["adversarial_loss"]) / self.train_hist["adversarial_loss"][0], label="adversarial_loss")
        plt.plot(self.train_hist["step_number"],
                 np.asarray(self.train_hist["contextual_loss"]) / self.train_hist["contextual_loss"][0], label="contextual_loss")
        plt.plot(self.train_hist["step_number"],
                 np.asarray(self.train_hist["encoder_loss"]) / self.train_hist["encoder_loss"][0], label="encoder_loss")
        plt.yscale("log")
    ...
    def threshold_and_MCC_plot(self):
        ...
        plt.xlabel("Steps")
        plt.plot(self.train_hist["step_number"], self.train_hist["MCC"], "y", label="MCC")
        plt.legend(loc="lower left")
        plt.ylabel("MCC")
        plt.ylim(bottom=0)

        ax2 = ax.twinx()
        plt.plot(self.train_hist["step_number"], self.train_hist["threshold"], "b", label="threshold")
    ...
    def confusion_matrix(self):
        ...
        for i in indizes:
            ...
            if self.validation:
                ax.set_title(f"Rowwise normalized confusion matrix @ step {self.train_hist['step_number'][i]}")
            else:
    ...
    def show_pixXpix_samples(self):
        ...
        for i in indizes:
            ...
            axs[0, 0].set_title(f"x_normal - step {self.train_hist['step_number'][i]}")
            plt.colorbar(im, ax=axs[0, 0])
            im = axs[1, 0].imshow(x_hat_normal_samples[i][0])
            axs[1, 0].set_title(f"x_hat_normal - step {self.train_hist['step_number'][i]}")
            plt.colorbar(im, ax=axs[1, 0])
            im = axs[0, 1].imshow(x_unnormal_samples[i][0])
            axs[0, 1].set_title(f"x_anomaly - step {self.train_hist['step_number'][i]}")
            plt.colorbar(im, ax=axs[0, 1])
            im = axs[1, 1].imshow(x_hat_unnormal_samples[i][0])
            axs[1, 1].set_title(f"x_hat_anomaly - step {self.train_hist['step_number'][i]}")
