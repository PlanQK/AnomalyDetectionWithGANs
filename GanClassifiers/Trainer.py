...
class Trainer:
    ...
    def __init__(
        ...
        Args:
            ...
            updateInterval (int, optional): Determines how often model performance is checked against the validation
                    ...
                    Defaults to 50.
            gpWeight (float, optional): Weight of the Wasserstein-GAN penalty added to the discriminator loss.
            adv-, con-, enc_loss_weight (float/int, optional): Weights of the generator loss components adversarial,
                    contextual and encoding loss.
        """
        ...
        self.validation = True if self.envMgr["trainOrpredict"] == "train" else False
        self.opt_disc = tf.keras.optimizers.Adam(beta_1=0.5, lr=float(self.envMgr["discTrainingRate"]))
        self.opt_gen = tf.keras.optimizers.Adam(beta_1=0.5, lr=float(self.envMgr["genTrainingRate"]))
        self.latent_dim = self.envMgr["latentDim"]
        self.n_steps = self.envMgr["trainingSteps"]
        self.step_counter = 0
        self.batchSize = self.envMgr["batchSize"]
        self.discriminatorIterations = self.envMgr["discriminatorIterations"]
        self.updateInterval = self.envMgr["validationInterval"]
        self.validationSamples = min(self.envMgr["validationSamples"], len(self.Data.test_data_normal),
                                     len(self.Data.validation_data_unnormal))
        self.gpWeight = self.envMgr["gpWeight"]
        ...
        self.d_loss = 0
        self.best_mcc = -1.
        self.train_hist = {"stepNumber": [], "totalRuntime": 0, "Runtime_per_step": 0, "generator loss": [],
                           "adv loss": [], "con loss": [], "enc loss": [],
    ...
    def train(self):
        """Run the training procedure. The first validation and logging of losses happens after validationInterval
        ...
        for _ in tqdm(range(int(self.n_steps)), desc=self.__str__()):
            ...
            if (int(self.step_counter) % int(self.updateInterval)) == 0:
                ...
                metrics["generator loss"] = self.g_loss
                metrics["discriminator loss"] = self.d_loss
                metrics["adv loss"] = self.adv_loss
                metrics["con loss"] = self.con_loss
                metrics["enc loss"] = self.enc_loss
                for key, value in metrics.items():
                    self.train_hist[key].append(value)
                self.train_hist["stepNumber"].append(self.step_counter)
                print(f"\nMCC: {self.train_hist['MCC'][-1]}, Gen loss: {self.g_loss}, Disc loss: {self.d_loss}")
        toc = time.perf_counter()

        self.train_hist["totalRuntime"] = toc - tic
        if self.n_steps != 0:
    ...
    def _step(self):
        ...
        """
        x = self.Data.get_train_data(self.batchSize)
        for i in range(int(self.discriminatorIterations)):
            with tf.GradientTape(persistent=True) as tape:
                ...
                d_loss = d_cost + gp * float(self.gpWeight)

                # generator losses
                if i == (int(self.discriminatorIterations) - 1):
    ...
    def gradient_penalty(self, x, x_hat):
        ...
        alpha = tf.random.uniform(
            [int(self.batchSize), 1], 0.0, 1.0, dtype=tf.dtypes.float64
        )
        diff = x_hat - x
    ...
    def calculateMetrics(self, step=0, validation_or_test="validation"):
        ...
        if validation_or_test == "validation":
            x_normal, x_unnormal = self.Data.get_validation_data(batchSize=int(self.validationSamples))
        ...
        if validation_or_test == "validation":
            ...
            if self.best_mcc < res["MCC"]:
                ...
                self.Classifier.save(step=step, MCC=res["MCC"], threshold=res["best_threshold"], overwrite_best=True)
    ...
    def optimize_anomaly_threshold_and_get_metrics(self, enc_loss_normal, enc_loss_unnormal, validation=True):
        ...
        # during testing, only look for the index of the last sample, whose anomaly score falls below the threshold
            ...
            best_threshold = self.Classifier.threshold
            best_index = 0
            for i in range(1, len(sorted_complete_set)):
                if sorted_complete_set[i][0] > best_threshold:
                    continue
                else:
                    best_index = i
...
class QuantumDecoderTrainer(Trainer):
    ...
    def calculateMetrics(self, step=0, validation_or_test="validation"):
        ...
        mae = tf.keras.losses.MeanAbsoluteError(reduction=tf.keras.losses.Reduction.NONE)
        if validation_or_test == "validation":
            x_normal, x_unnormal = self.Data.get_validation_data(batchSize=self.validationSamples)
        elif validation_or_test == "test":
            x_normal, x_unnormal = self.Data.get_test_data()
