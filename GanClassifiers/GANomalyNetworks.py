"""This file holds classes which contain the neural networks of the GANomaly model.
"""
import logging

import sympy

from .slimtasq.GANomalyAnsatz import GANomalyAnsatz
from .slimtasq.GANomalyCost import GANomalyCost
...
from GanClassifiers.QuantumCircuits import CompleteRotationCircuitIdentity, CompleteRotationCircuitRandom, \
...
class QuantumDecoderNetworks(Classifier):
    ...
    def __init__(self, data):
        ...
        """
        super().__init__(data=data)
        self.repetitions = self.envMgr["num_shots"]
        self.qubits = cirq.GridQubit.rect(1, self.latent_dim)
        self.quantum_weights = None
        self.quantum_circuit = None
        self.qc_type = self.envMgr["qcType"]
        self.totalNumCycles = self.envMgr["quantumDepth"]
        self.auto_encoder = self.getEncoder()
        self.auto_decoder = self.getDecoder()
        self.encoder = self.getEncoder()
        self.discriminator = self.getDiscriminator()
